...
@login_required
def top():
    memo_list = get_db().execute("select id,title,body,create_user from memo").fetchall()
    izin_list = get_db().execute("select name,word from izin").fetchall()
    selected_items = []
    for i in range(len(memo_list)):   
        selected_items.append(random.choice(izin_list))
    return render_template('index.html',memo_list=memo_list,items=selected_items)
